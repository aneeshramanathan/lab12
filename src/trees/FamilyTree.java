...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		/*substring of line that starts just after colonIndex and goes through the end of
        the line. You'll use a different version of substring().*/
		String childrenString = line.substring(colonIndex+1).trim(); 
		
		/*Check the API for details. The result will be an array
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Node 1 does not exist");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Node 2 does not exist");
